...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model-preset", default="mistral-7b", help="Preset modelu lokalnego, np. first-local, tinyllama, mistral-7b")
    ...
    parser.add_argument("--judge-model", default="Qwen/Qwen3-1.7B", help="Model do użycia jako sędzia")
    ...
    parser.add_argument("--no-defensive-system", action="store_true", help="Ustawia neutralny system prompt: 'Be a helpfull AI assistant'")
    parser.add_argument("--enable-heuristics", action="store_true", help="Włącz heurystyczne wykrywanie prompt injection (nie modyfikuje odpowiedzi domyślnie)")
    parser.add_argument("--heuristic-reject", action="store_true", help="Jeżeli heurystyka wykryje problem, wymuś 'reject' (symulacja aktywnej obrony)")
    parser.add_argument("--write-summary", action="store_true", help="Zapisz plik summary JSON (domyślnie wyłączone — metryki policzymy później)")
    parser.add_argument("--no-sanitize", action="store_true", help="Wyłącz sanitizację promptów wejściowych")
    parser.add_argument("--stratify-attacks", type=bool, default=True, help="Stratyfikuj testy według typów ataków")
    return parser
...
def create_adapter(args: argparse.Namespace):
    ...
    print(f"args.model_preset: {args.model_preset} | Ładowanie adaptera modelu: {preset.name} (model_name={model_name}, load_in_8bit={args.load_in_8bit}, load_in_4bit={args.load_in_4bit})")
...
def main() -> int:
    ...
    examples = load_examples(dataset, limit=args.limit, stratify_attacks=args.stratify_attacks)
    ...
    system_prompt = ""
    if not args.no_defensive_system:
        # domyślny defensywny system prompt z modułu
        from prompt_injection_models import create_default_system_prompt

        system_prompt = create_default_system_prompt()
    else:
        system_prompt = "Be a helpfull AI assistant"
    ...
    records = run_benchmark(
        ...
        heuristic_reject=args.heuristic_reject,
    )
    save_records(records, Path(args.output_dir), write_summary=args.write_summary)
